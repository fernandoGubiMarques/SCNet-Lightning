...
class SCNet(nn.Module):
    ...
    This model performs music source separation by leveraging STFT-based feature extraction, convolutional encoding/decoding, and a separation network.
    """
    ...
    def __init__(
        ...
        stft_config: dict[str, int|bool],
        # SD/SU (Sparse Downsampling/Unfolding) layer parameters
        band_configs: dict[str, dict],
        # Convolution Module parameters
        conv_depths: list[int],
        conv_config: dict[str, int],
        # Dual-path RNN parameters
        num_dplayer: int,
        expand: int,
    ):
        super().__init__()
        ...
        # STFT configuration dictionary
        self.hop_length = self.stft_config["hop_length"]
        
        # Define encoder and decoder modules as lists
        self.encoder = nn.ModuleList()
        self.decoder = nn.ModuleList()
        
        # Construct encoder and decoder blocks
        for index in range(len(dims) - 1):
            # Encoder consists of Sparse Downsampling (SD) blocks
            enc = SDblock(
            ...
            # Decoder consists of Fusion and Sparse Unfolding (SU) layers
            dec = nn.Sequential(
            ...
            self.decoder.insert(0, dec)  # Reverse order for decoding

        # Separation network processes encoded features before decoding
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Ensure STFT produces an even-length time dimension for efficient RFFT computation
        padding = self.hop_length - x.shape[-1] % self.hop_length
        if (x.shape[-1] + padding) // self.hop_length % 2 == 0:
            padding += self.hop_length
        x = F.pad(x, (0, padding))
        
        # Apply STFT to convert time-domain audio to time-frequency representation
        L = x.shape[-1]
        x = x.reshape(-1, L)  # Flatten batch and channel dimensions for STFT
        x = torch.stft(x, **self.stft_config, return_complex=True)
        x = torch.view_as_real(x)  # Convert complex output to real-valued representation
        
        # Reshape STFT output to match model expectations
        ...
        # Normalize STFT features for stability
        B, C, Fr, T = x.shape  # Batch, Channels, Frequency, Time
        mean = x.mean(dim=(1, 2, 3), keepdim=True)
        std = x.std(dim=(1, 2, 3), keepdim=True)
        x = (x - mean) / (1e-5 + std)
        
        # Initialize deques to store intermediate encoder outputs for later use in decoding
        save_skip = deque()
        save_lengths = deque()
        save_original_lengths = deque()
        
        # Encoder: Apply Sparse Downsampling (SD) layers
        ...
        # Separation network processes the encoded features
        x = self.separation_net(x)
        
        # Decoder: Apply Fusion and Sparse Unfolding (SU) layers
        ...
        n = self.dims[0]  # Number of output channels
        x = x.view(B, n, -1, Fr, T)
        x = x * std[:, None] + mean[:, None]  # Reapply normalization statistics
        x = x.reshape(-1, 2, Fr, T).permute(0, 2, 3, 1)
        x = torch.view_as_complex(x.contiguous())  # Convert back to complex representation
        x = torch.istft(x, **self.stft_config)  # Inverse STFT to reconstruct time-domain signal
        
        x = x.reshape(B, len(self.sources), self.audio_channels, -1)
        x = x[:, :, :, :-padding]  # Remove padding to match original input length
