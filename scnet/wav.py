...
class Wavset(Dataset):
    ...
    def __init__(
        ...
        for song_dir in tqdm(song_dirs, f"Computing {partition_name} set metadata"):
            ...
            # Load example stem
            audio, _ = torchaudio.load(song_dir / f"{stems[0]}.{extension}")

            # Compute stats
            audio_len = audio.shape[-1]
            if segment_samples > 0:
                segment_starts = list(range(0, audio_len - segment_samples, stride_samples))
            else:
                segment_starts = [0]
            segment_durations = [segment_samples] * len(segment_starts)
            dirs = [song_dir] * len(segment_starts)

            self.metadata += list(zip(segment_starts, segment_durations, dirs))
        
    def __len__(self):
        return len(self.metadata)
    
    def __getitem__(self, i):
        start, duration, dir = self.metadata[i]
        dir: Path
...
#             return example, meta['mean'], meta['std']
